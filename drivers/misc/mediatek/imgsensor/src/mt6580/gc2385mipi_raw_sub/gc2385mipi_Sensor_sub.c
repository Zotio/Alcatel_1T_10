...
#include "gc2385mipi_Sensor_sub.h"
...
static imgsensor_info_struct imgsensor_info = {
    .sensor_id = GC2385MIPI_SENSOR_SUB_ID,//2385MIPI_SENSOR_SUB_ID,        //record sensor id defined in Kd_imgsensor.h
...
static imgsensor_struct imgsensor = {
    .mirror = 3,//IMAGE_NORMAL,                //mirrorflip information
...
{	
	...
	if((ANALOG_GAIN_1<= iReg)&&(iReg < ANALOG_GAIN_2))
	{
		...
		printk("GC2385MIPI analogic gain 1x, GC2385MIPI add pregain = %d\n",temp);
	}
        ...
        do {
            *sensor_id = return_sensor_id()&0xffef;
            if (*sensor_id == imgsensor_info.sensor_id) {
                printk("LQ ---> gc2385_sub i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                return ERROR_NONE;
            }
            printk("LQ --> gc2385_sub Read sensor id fail, write id: 0x%x, id: 0x%x,imgsensor_info.sensor_id:0x%x\n", imgsensor.i2c_write_id,*sensor_id,imgsensor_info.sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 2;
...
UINT32 GC2385MIPI_RAW_SUB_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{

...
static void set_dummy(void)
{
	...
	vb = (vb < 1024) ? 1024 : vb;//test for 35m
        	...
        	do {
            	*sensor_id = return_sensor_id();
            	if (*sensor_id == imgsensor_info.sensor_id) {
                	printk("LQ --->gc5025  i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                	return ERROR_NONE;
            	}
            	printk("LQ -->  gc5025 Read sensor id fail, write id: 0x%x, id: 0x%x,imgsensor_info.sensor_id=0x%x\n", imgsensor.i2c_write_id,*sensor_id,imgsensor_info.sensor_id);
            	retry--;
        	} while(retry > 0);
        	i++;
        	retry = 2;

...
#include "sp250amipi_Sensor_sub.h"
#ifdef XUNHU_LPS_TEKHW_SUPPORT
#include <teksunhw.h>/*add by xunhu andy andy20160720 at 22:49*/
static int tekhw_sp250a_mirror = 0xff;/*add by xunhu andy andy20160720 at 22:48*/
static int tekhw_sp250a_color =0xff;
...
static imgsensor_info_struct imgsensor_info = {
    .sensor_id = 0x250b,//SP250AMIPI_SENSOR_ID,
...
static kal_uint16 set_gain(kal_uint16 gain)
{
		...
		{   
             ...
             //iReg = (gain%BASEGAIN)/(0x10/BASEGAIN) + iReg;
...
write_cmos_sensor(0x8a,0x0f);
            ...
            printk("sp250a:0x9d=%x\n",read_cmos_sensor(0x9d));
            printk("sp250a:0xa0=%x\n",read_cmos_sensor(0xa0));
            printk("sp250a:0xa1=%x\n",read_cmos_sensor(0xa1));
            printk("sp250a:0xad=%x\n",read_cmos_sensor(0xad));
            printk("sp250a:0xae=%x\n",read_cmos_sensor(0xae));
            printk("sp250a:0xaf=%x\n",read_cmos_sensor(0xaf));
            printk("sp250a:0xb1=%x\n",read_cmos_sensor(0xb1));
            printk("sp250a:0xac=%x\n",read_cmos_sensor(0xac));
            printk("sp250a:0xfd=%x\n",read_cmos_sensor(0xfd));
            printk("sp250a:0xfc=%x\n",read_cmos_sensor(0xfc));
            printk("sp250a:0xfe=%x\n",read_cmos_sensor(0xfe));
            printk("sp250a:0xf9=%x\n",read_cmos_sensor(0xf9));
            printk("sp250a:0xfa=%x\n",read_cmos_sensor(0xfa));
            printk("sp250a:0x3f=%x\n",read_cmos_sensor(0x3f));
        ...
        do {
            *sensor_id = return_sensor_id();
            if (*sensor_id == imgsensor_info.sensor_id) {
				*sensor_id = SP250AMIPI_SENSOR_SUB_ID;
                printk("LQ --> sp250a_sub gpw i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                return ERROR_NONE;
            }
            printk("LQ --> sp250a_sub gpw Read sensor id fail, write id:0x%x id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 2;
    }
    if (*sensor_id != SP250AMIPI_SENSOR_ID) {
...
static kal_uint32 open(void)
{
	kal_uint8 i = 0;
	 ...
	 if (SP250AMIPI_SENSOR_ID != sensor_id)
		 return ERROR_SENSOR_CONNECT_FAIL;
                      ...
                      MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	#ifdef XUNHU_LPS_TEKHW_SUPPORT
    if (0xff != tekhw_sp250a_mirror)
    {
	    set_mirror_flip(tekhw_sp250a_mirror);
    }
#endif
    return ERROR_NONE;
}    /*    preview   */
...
{
    tekhw_camera_data* tekhw_sp250a_ptr = tekhw_get_appointed_camera_ptr(SENSOR_DRVNAME_SP250A_MIPI_RAW);/*add by xunhu andy andy20160720 at 22:56*/
    if ((tekhw_sp250a_ptr != NULL) && (tekhw_sp250a_ptr->set_mirror.size > 0))
    {
        tekhw_sp250a_mirror = *(tekhw_u16*)tekhw_sp250a_ptr->set_mirror.buffer;
    }
    if ((tekhw_sp250a_ptr != NULL) && (tekhw_sp250a_ptr->set_color.size > 0))
    {
		tekhw_sp250a_color= *(tekhw_u16*)tekhw_sp250a_ptr->set_color.buffer;
    }
}
#endif
...
if(tekhw_times == 0)
	...
	if(tekhw_sp250a_color!=0xff)
	{
		sensor_info->SensorOutputDataFormat = tekhw_sp250a_color;
	}
tekhw_times =1;
#endif
...
UINT32 SP250A_MIPI_RAW_SUB_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{

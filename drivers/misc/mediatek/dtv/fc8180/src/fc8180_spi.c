...
static struct spi_driver fc8180_spi_driver = {
	.driver = {
		...
		.of_match_table = of_match_ptr(fc8180_match_table),
	...
	, u8 *txbuf, u16 tx_length, u8 *rxbuf, u16 rx_length)
{
	...
	else
	{
		...
		if ((rxbuf != NULL) && (rx_length > 0)) {
			...
			print_log(0, "[FC8180] fc8180_spi_write_then_read 0x%x, 0x%x, %d\n", rxbuf[0], rxbuf[rx_length - 1], rx_length);
			}
	}
	return res;
}
...
s32 fc8180_spi_init(HANDLE handle, u16 param1, u16 param2)
{
	...
	mt_set_gpio_mode(0,1);
	mt_set_gpio_mode(1,1);
	mt_set_gpio_mode(2,1);
	mt_set_gpio_mode(3,1);

...
#define ZHENG_USE_PINCTRL 1

#define GT91XX_PROC_CONFIG_VERSION     "gt9xx_config"
#define GT91XX_PROC_FW_VERSION     "gt9xx_fw_version"
#define GT91XX_PROC_SENSOR_ID    "gt9xx_sensor_id"//add by lsk
...
//add by LQ 
static struct proc_dir_entry *gt91xx_fw_version;
static struct proc_dir_entry *gt91xx_sensor_id;
static struct proc_dir_entry *gt91xx_config_version;
//end by LQ
...
static u8 drv_cfg_version =-1;
static int tpd_sensor_id = -1;//add by lsk
//end by LQ
...
static ssize_t gt91xx_sensor_id_read_proc(struct file *file, char *buffer, size_t count, loff_t *ppos){
	char *page = NULL;
	char *ptr = NULL;
	int len, err = -1;
//	char temp_data[GTP_CONFIG_MAX_LENGTH + 2] = { 0 };
...
s32 gtp_init_panel(struct i2c_client *client)
{
	...
	(!cfg_info_len[3]) && (!cfg_info_len[4]) && (!cfg_info_len[5])) {
		...
		tpd_sensor_id = sensor_id;//add by lsk
	}
...
reset_proc:
    printk("zheng tpd_power_on\n");
	gtp_gpio_output(GTP_IRQ_GPIO, 0);
	gtp_gpio_output(GTP_RST_GPIO, 0);
	msleep(20);
    if(GTP_IRQ_GPIO == 5)
    {
       printk("zheng GTP_IRQ_GPIO is OK\n");
	}
	if(GTP_RST_GPIO == 2)
    {
       printk("zheng GTP_RST_GPIO is OK\n");
	}
...
static const struct file_operations gt91xx_sensor_id_fops = {
	//.write = gt91xx_sensor_id_write_proc,
	.read = gt91xx_sensor_id_read_proc
};


static const struct file_operations gt_config_version_fops = {
	//.write = gt91xx_config_version_write_proc,
...
static s32 tpd_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
...
#endif
    printk("zheng tpd_i2c_probe\n");
	client->addr = 0x14;   //add by zhengbichao 
	...
	printk("zheng tpd_i2c_probe 01\n");
	
	ret = gtp_init_panel(client);//temp modify by lsk 180410
	...
	gt91xx_sensor_id =proc_create(GT91XX_PROC_SENSOR_ID, 0777, NULL,&gt91xx_sensor_id_fops);
	if (gt91xx_sensor_id == NULL)
		GTP_ERROR("create_proc_entry %s failed\n",GT91XX_PROC_SENSOR_ID);
...
#endif
#ifdef CONFIG_GTP_GESTURE_WAKEUP
    printk("zheng enable gesture_data\n");
	gtp_extents_init();
#if 0
	input_set_capability(tpd->dev, EV_KEY, KEY_F2);
	input_set_capability(tpd->dev, EV_KEY, KEY_F3);   KEY_POWER
#else
...
{
	TPD_DEBUG_PRINT_INT;
	//printk("zheng tpd_interrupt_handler\n");
	if (irq_enabled) {
		irq_enabled = false;
...
static int touch_event_handler(void *unused)
{
	...
	do {
		...
		//	printk("zheng Interrupt gesture event handled, ret = %d",ret);
...
static s8 gtp_enter_sleep(struct i2c_client *client)
{
	...
	printk("GTP enter sleep by poweroff!");
	return 0;
#else
	{
		s8 ret = -1;
...
static s8 gtp_wakeup_sleep(struct i2c_client *client)
{
	...
	while (retry++ < 10) {
#ifdef CONFIG_GTP_GESTURE_WAKEUP
     printk("gtp_wakeup_sleep gesture_data.enabled = %d\n",gesture_data.enabled);
		if (gesture_data.enabled) {		
...
static void tpd_suspend(struct device *h)
{
	...
	printk("zheng 2tpd_suspend gesture_data.enabled = %d\n",gesture_data.enabled);
#ifdef CONFIG_GTP_PROXIMITY
	if (tpd_proximity_flag == 1)
		return;
#endif
...
#endif
    printk("zheng tpd_suspend 02\n");
#ifdef CONFIG_GTP_GESTURE_WAKEUP
	if (gesture_data.enabled) {
		printk("zheng tpd_suspend 03\n");
		ret = gtp_enter_doze();
		gtp_irq_enable();
...
#ifndef CONFIG_GTP_GESTURE_WAKEUP
    printk("CONFIG_GTP_GESTURE_WAKEUP gtp_irq_enable \n");
	gtp_irq_enable();
#endif

...
#define CONFIG_GT9XX_10INCH
#ifdef CONFIG_GT9XX_10INCH
#include "gt9xx_config_10.h"
#else
...
s32 gtp_read_version(struct i2c_client *client, u16 *version)
{
	...
	if (ret < 0) {
		printk("zheng GTP read version failed\n");
		return ret;
	}
	...
	if (buf[5] == 0x00) {
		printk("zheng IC VERSION: %c%c%c_%02x%02x\n", buf[2], buf[3], buf[4], buf[7], buf[6]);
	} else {
		if (buf[5] == 'S' || buf[5] == 's')
			chip_gt9xxs = 1;
		printk("zheng IC VERSION:%c%c%c%c_%02x%02x\n",
...
static s32 gtp_get_info(struct i2c_client *client)
{
	...
	//printk("LQ --> X_MAX = %d, Y_MAX = %d, TRIGGER = 0x%02x", abs_x_max, abs_y_max, int_type);
...
static s32 gtp_init_panel(struct i2c_client *client)
{
...
#endif
	{
		...
		config[cfg_len] = (~check_sum) + 1;
#endif
		printk("zheng X_MAX = %d, Y_MAX = %d, TRIGGER = 0x%02x",
...
{
	//printk("LQ ft911 ============= tpd_down x = %d , y = %d\n",x,y);
	if ((!size) && (!id)) {

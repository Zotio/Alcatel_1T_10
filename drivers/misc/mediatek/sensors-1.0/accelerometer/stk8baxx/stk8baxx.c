...
#include <mt-plat/mt_gpio.h> //LQ
#define IRQ_PIN	(GPIO4 | 0x80000000)
*/
/*----------------------------------------------------------------------------*/
...
static struct acc_hw accel_cust_stk8baxx;
static struct acc_hw *stk8baxx_hw = &accel_cust_stk8baxx;
...
//added by dongming
//>>>
static int s_skip_values_after_enable = 0;
//<<<
...
static int STK8BAXX_SetPowerMode(struct i2c_client *client, bool enable)
{
	...
	//added by dongming
	s_skip_values_after_enable = enable;
...
static int gsensor_get_data(int *x, int *y, int *z, int *status)
{
	...
	*status = SENSOR_STATUS_ACCURACY_MEDIUM;
	...
	if (atomic_read(&obj_i2c_data->trace) & ADX_TRC_IOCTL)
		GSE_LOG("x = %d, y = %d, z = %d\n", *x, *y, *z);
#else				/* #ifdef CONFIG_CUSTOM_KERNEL_SENSORHUB */
    if ( s_skip_values_after_enable ) {
		s_skip_values_after_enable = 0;
		return -1;
	}
	mutex_lock(&gsensor_mutex);
...
{
	const char *name = "mediatek,STK8BAXX";
	GSE_FUN();
	stk8baxx_hw = check_accel_dts_func(name, stk8baxx_hw);
	if(stk8baxx_hw == NULL)
	{
			GSE_ERR("stk8baxx_hw is null");
			return 0;
	}

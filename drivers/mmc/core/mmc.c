 ...
 */
char *emmc_density[] = {"8G","16G","Unknown"};
char *emmc_vendor[] = {"Sandisk","FORESEE","Unknown"};
char *emmc_name[] = {"SDINADF4-16G","NCEMASKG-16G","Unknown"};
...
static int mmc_decode_cid(struct mmc_card *card)
{
	...
	if (card->cid.manfid == 0x45) {
		memcpy(card->cid.eMMC_name,emmc_name[0],sizeof(card->cid.eMMC_name));
		memcpy(card->cid.eMMC_vendor,emmc_vendor[0],sizeof(card->cid.eMMC_vendor));
		memcpy(card->cid.eMMC_density,emmc_density[1],sizeof(card->cid.eMMC_density));
	} else if (card->cid.manfid == 0x88){
		memcpy(card->cid.eMMC_name,emmc_name[1],sizeof(card->cid.eMMC_name));
		memcpy(card->cid.eMMC_vendor,emmc_vendor[1],sizeof(card->cid.eMMC_vendor));
		memcpy(card->cid.eMMC_density,emmc_density[1],sizeof(card->cid.eMMC_density));
	} else {
		memcpy(card->cid.eMMC_name,emmc_name[2],sizeof(card->cid.eMMC_name));
		memcpy(card->cid.eMMC_vendor,emmc_vendor[2],sizeof(card->cid.eMMC_vendor));
		memcpy(card->cid.eMMC_density,emmc_density[2],sizeof(card->cid.eMMC_density));
	}
...
MMC_DEV_ATTR(eMMC,"%s:%s:%s\n",card->cid.eMMC_name,card->cid.eMMC_vendor,card->cid.eMMC_density);

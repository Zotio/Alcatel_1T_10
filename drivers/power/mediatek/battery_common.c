...
#include <linux/proc_fs.h>  //add by LQ
...
static struct proc_dir_entry *otg_status; //add by LQ
 ...
 *  Global Variable
 */
int ntc_temp = 1; //add by LQ ,11.28
EXPORT_SYMBOL(ntc_temp);
int cooling_status=0;
EXPORT_SYMBOL(cooling_status);
/*charge ic vendor 0:HL7007 1:PSC5415A*/
int ic_vendor=-1;
EXPORT_SYMBOL(ic_vendor);
...
static struct battery_data battery_main = {
	.psy = {
		...
		.set_property = battery_set_property, //add by LQ ,11.28
		.property_is_writeable = battery_is_writeable,//add by LQ ,11.28
		},
...
//long time suspend battery percentage calibration,,sync with pixi473g
static int sync_real_soc_to_uisoc_rtc = 0;
static int sync_real_soc_to_uisoc_rtc_back = 0;
...
//[BUGFIX]-Mod-BEGIN by SCDTABLET.zhengding.chen@tcl.com,2016/11/29,PR3621040
//long time suspend battery percentage calibration,,sync with pixi473g
				sync_real_soc_to_uisoc_rtc_back=sync_real_soc_to_uisoc_rtc;
//[BUGFIX]-Mod-END by SCDTABLET.zhengding.chen@tcl.com,2016/11/29,PR3621040
				BMT_status.UI_SOC--;
				timer_counter = 0;
			}
...
void mt_battery_GetBatteryData(void)
{
...
#ifndef CONFIG_MTK_CLOSE_NTC_DET
...
#ifdef CONFIG_MTK_CLOSE_NTC_DET
		BMT_status.temperature = 25; 
#else
		if(0==ntc_temp){
			BMT_status.temperature = 25;
			//printk("LQ ---> %s BMT_status.temperature = 25\n",__func__);
		}		
		else{
			BMT_status.temperature = mt_battery_average_method(BATTERY_AVG_TEMP, &batteryTempBuffer[0], temperature,
					      &temperature_sum, batteryIndex);
			//printk("LQ ---> %s BMT_status.temperature = %d\n",__func__,BMT_status.temperature);
		}
#endif
	}
	...
	battery_update(&battery_main);//add by LQ
...
unsigned int charging_enable_flag = KAL_FALSE;
...
//add by lsk  discharging 180426
if((BMT_status.temperature >= 42)&&(BMT_status.bat_vol >= 4100)&&(BMT_status.charger_exist)&&(!charging_enable_flag)){	
//		bat_data->BAT_CAPACITY = 0;
		cmd_discharging = 1;
		charging_enable_flag = KAL_TRUE;

		battery_charging_control(CHARGING_CMD_SET_ERROR_STATE, &cmd_discharging);
		printk("[bat][lsk] discharging %d  %d   %d exist=%d\n",cmd_discharging,BMT_status.temperature,BMT_status.bat_vol,BMT_status.charger_exist);	
	}else if(((BMT_status.bat_vol <4100)&&(BMT_status.charger_exist)&&(charging_enable_flag))||
	((BMT_status.temperature >= 42)&&(BMT_status.charger_exist)&&(charging_enable_flag))){
		//charging_enable = false;
		//battery_charging_control(CHARGING_CMD_ENABLE, &charging_enable);
		charging_enable_flag = KAL_FALSE;
		cmd_discharging = 0;
		battery_charging_control(CHARGING_CMD_SET_ERROR_STATE, &cmd_discharging);
		
		printk("[bat][lsk] charging_enable temp=%d vol=%d \n",BMT_status.temperature,BMT_status.bat_vol);
...
static ssize_t otg_status_read_proc(struct file *file, char *buffer, size_t count, loff_t *ppos)
{
	char *page = NULL;
	char *ptr = NULL;
	int len, err = -1;
...
static const struct file_operations otg_status_fops = {
	//.write = gt91xx_fw_version_write_proc,
	.read = otg_status_read_proc
};
...
static int battery_probe(struct platform_device *dev)
{
	...
	if(ic_vid){
		//printk("LQ --> %s ,detect charger ic:PSC5415A\n",__func__);
		ic_vendor=1;
	}else{
		//printk("LQ --> %s ,detect charger ic:HL7007\n",__func__);
		ic_vendor=0;
	}
	...
	otg_status =proc_create("otg_status", 0777, NULL,&otg_status_fops);
	if (otg_status == NULL)
		printk("%s create_proc_entry otg_status failed\n",__func__);
...
//long time suspend battery percentage calibration,,sync with pixi473g
       ...
       sync_real_soc_to_uisoc_rtc=rtc_read_hw_time();

       if(sync_real_soc_to_uisoc_rtc_back==0)
               sync_real_soc_to_uisoc_rtc_back=sync_real_soc_to_uisoc_rtc;

       battery_log(BAT_LOG_CRTI, "sync_real_soc_to_uisoc_rtc=%d,sync_real_soc_to_uisoc_rtc_back=%d\n", \
                       sync_real_soc_to_uisoc_rtc,sync_real_soc_to_uisoc_rtc_back);
       battery_log(BAT_LOG_CRTI, "UI_SOC=%d,SOC=%d\n", \
                       BMT_status.UI_SOC,BMT_status.SOC);

       if((sync_real_soc_to_uisoc_rtc-sync_real_soc_to_uisoc_rtc_back)>60*60)  //1hour
       {
               if ((BMT_status.UI_SOC > BMT_status.SOC) && ((BMT_status.UI_SOC != 1))) {
                       BMT_status.UI_SOC=BMT_status.SOC;

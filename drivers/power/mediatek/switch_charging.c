...
extern int ntc_temp; //add by LQ
extern int ic_vendor;
...
static unsigned int charging_full_check(void)
{
	...
	chg_current = battery_meter_get_charging_current();
if(1==ic_vendor){
       if((BMT_status.bat_full == KAL_TRUE)&&(BMT_status.bat_in_recharging_state == KAL_FALSE))
       {
               ...
               if ((status == KAL_FALSE)&& (BMT_status.bat_vol <= batt_cust_data.recharging_voltage))
               {
                       g_full_check_count++;
...
#endif
//add by LQ
	//printk("LQ --> %s cooling_status=%d\n",__func__,cooling_status);
	if((temp>=42)&&(temp<=50)){
		g_temp_CC_value=50000; //500mA
...
//end by LQ
	...
	//battery_log(BAT_LOG_CRTI,
		printk("LQ ---> %s,  CC mode charging : %d, input current = %d\n",__func__,
...
static void mtk_select_cv(void)
{
	...
	if (batt_cust_data.high_battery_voltage_support)
		cv_voltage = BATTERY_VOLT_04_400000_V;//BATTERY_VOLT_04_380000_V;/*BATTERY_VOLT_04_340000_V;*/
	else
		cv_voltage = BATTERY_VOLT_04_200000_V;

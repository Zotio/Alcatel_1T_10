...
static int __init mt65xx_leds_init(void)
{
	int ret;
#ifdef CONFIG_BACKLIGHT_ZS_DIFF
	int lcd_id1=-1;        // gpio1   gpio6    gpio_1   gpio_6
	int lcd_id2=-1; 
	int gpio_id1=6; 
	int gpio_id2=1;
#endif
...
/* platform_device_unregister(&mt65xx_leds_device); */
...
#ifdef CONFIG_BACKLIGHT_ZS_DIFF
		#if 1
		mt_set_gpio_mode(gpio_id1,0);
		mt_set_gpio_mode(gpio_id2,0);
		mt_set_gpio_dir(gpio_id1,0);
		mt_set_gpio_dir(gpio_id2,0);
		lcd_id1 = mt_get_gpio_in(gpio_id1);
		lcd_id2 = mt_get_gpio_in(gpio_id2);
		#else
		...
		printk("zheng -->  lcd_id1=%d,lcd_id2=%d\n",lcd_id1,lcd_id2);
		if((lcd_id1 == 0)&&(lcd_id2 == 0)){
        zs_brightness = 1;
		printk("led : LCM is ZS101NN3401Q3H10II zs_brightness = %d\n",zs_brightness);
		}
#endif

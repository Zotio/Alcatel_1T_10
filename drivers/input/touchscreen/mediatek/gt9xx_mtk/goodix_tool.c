...
static char *proc_parent_dir = "tct_debug";
static struct proc_dir_entry *goodix_proc_parent_entry;
...
s32 init_wr_node(struct i2c_client *client)
{
    ...
    goodix_proc_parent_entry = proc_mkdir(proc_parent_dir, NULL);
    if (!goodix_proc_parent_entry) {
        GTP_ERROR("faild create parent dir: %s\n", proc_parent_dir);
        return FAIL;
    }

	tool_set_proc_name(procname);
	goodix_proc_entry = proc_create(procname, 0666, goodix_proc_parent_entry, &tool_ops);

...
//#ifdef CONFIG_PSC5415A_CHARGER
static void chg_set_gpio_output(unsigned int GPIO, unsigned int output)
{
...
static u32 charging_enable(void *data)
{
	...
	if (KAL_TRUE == enable) {
		...
		if(1==ic_vendor)
			chg_set_gpio_output(9 , 0); //disable charger ic PSC5415A
	...
	if (current_value <= CHARGE_CURRENT_350_00_MA) {
		...
		if(1==ic_vendor){	
			...
			if(current_value == CHARGE_CURRENT_500_00_MA) {
				register_value = 0x1;	//psc5415 USB mode charging current set to 785ma (56mohm)
			}
		}else{
			array_size = GETARRAYNUM(CS_VTH);
...
static u32 charging_set_input_current(void *data)
{
	...
	if(1==ic_vendor){
		if (*(u32 *) data == CHARGE_CURRENT_500_00_MA) {
			register_value = 0x1;	//psc5415 USB mode limit set to 500mA
		}
	}

...
{
	s32 ret = 0;
	printk("zheng gtp_gesture_data_write\n");
	...
	printk("zheng <==> gesture enabled:%d, ret:%d", gesture_data.enabled, ret);
    if(gesture_data.enabled == 48){
		gesture_data.enabled = 0;
	}
	else{
		gesture_data.enabled = 1;
	}
	GTP_DEBUG("gesture enabled:%d, ret:%d", gesture_data.enabled, ret);
	printk("zheng gesture enabled:%d, ret:%d", gesture_data.enabled, ret);
...
s8 gtp_enter_doze(void)
{
    ...
    while(retry++ < 5) {
        ...
        if (ret < 0) {
            ...
            printk("failed to set doze flag into 0x8046, %d", retry);
            continue;
        }
...
s32 gesture_event_handler(struct input_dev * dev)
{
	...
	if (DOZE_ENABLED == gesture_data.doze_status) {
		...
		printk("0x%x = 0x%02X,0x%02X,0x%02X,0x%02X", GTP_REG_WAKEUP_GESTURE, doze_buf[0], doze_buf[1], doze_buf[2], doze_buf[3]);
...
#endif
			printk("zheng  doze enabled and get valid gesture data\n");
			return 2; /* doze enabled and get valid gesture data*/
		}
		printk("doze enabled, but no invalid gesutre data\n");
...
(_IOC_DIR(cmd) & _IOC_READ) ? "read" : (_IOC_DIR(cmd) & _IOC_WRITE) ? "write" : "-");*/
	...
	case GESTURE_ENABLE_TOTALLY:
		printk("ENABLE_GESTURE_TOTALLY\n");
		gesture_data.enabled = 1;
		break;

	case GESTURE_DISABLE_TOTALLY:
		printk("DISABLE_GESTURE_TOTALLY\n");
		gesture_data.enabled = 0;
		break;
	...
	case GESTURE_ENABLE_PARTLY:
		...
		gesture_data.enabled = 1;
		printk("ENABLE_GESTURE_PARTLY, gesture = 0x%02X, gesture_data.enabled = %d\n", value, gesture_data.enabled);
		break;

	case GESTURE_DISABLE_PARTLY:
		CLEARBIT(gestures_flag, (u8) value);
		printk("DISABLE_GESTURE_PARTLY, gesture = 0x%02X, gesture_data.enabled = %d\n", value, gesture_data.enabled);
		break;
...
s32 gtp_extents_init(void)
{
 ...
 //   gesture_data.enabled = 1;   //  enable gesture_data
	proc_entry = proc_create(GESTURE_NODE, 0777, NULL, &gtp_fops);
	if (proc_entry == NULL) {

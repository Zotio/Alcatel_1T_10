...
extern int ic_vendor; //add by LQ
/* ============================================================ // */
/* define */
/* ============================================================ // */
...
unsigned int g_battery_id_voltage[] = {500000, 2000000, -1};//0~0.5V for battery 0, 0.5~1V for battery 1, -1 for the last one (battery 2)
unsigned int TOTAL_BATTERY_NUMBER = 3;
int BATTERY_ID_CHANNEL_NUM = 12;//aux_in2 --->>>channel 12
/*
AUXIN0 channel 0
...
void fgauge_get_profile_id(void)
{
	...
	if (ret != 0){
		...
		printk("[lsk][fgauge_get_profile_id]id_volt read fail\n");
		}
	else
		{
		bm_print(BM_LOG_CRTI, "[fgauge_get_profile_id]id_volt = %d\n", id_volt);
		printk("[lsk][fgauge_get_profile_id]id_volt = %d\n", id_volt);
		}
...
#if 1//def CONFIG_EMDOOR_BATTERY_TYPE
	if(id_volt < (g_battery_id_voltage[0])){
		g_fg_battery_id = 1;//byd id_volt 124877
	}
	else
	{
		g_fg_battery_id = 0;
...
printk("[lsk][fgauge_get_profile_id]Battery id (%d)\n", g_fg_battery_id);
...
int __batt_meter_init_cust_data_from_dt(void)
{
	...
	if (g_fg_battery_id == 1) {
		__batt_meter_parse_node(np, "batt_temperature_table_byd_num", &num);

		idx = 0;
		while (!of_property_read_u32_index(np, "batt_temperature_table_byd", idx, &addr)) {
			idx++;
			if (!of_property_read_u32_index(np, "batt_temperature_table_byd", idx, &val)) {
				battery_log(BAT_LOG_CRTI, "batt_temperature_table_byd: addr: %d, val: %d\n",addr, val);
			}
...
if(g_fg_battery_id == 1)
	{
		printk(KERN_ERR"emdoor_SUCD id=%d\n\n",g_fg_battery_id);
	...
	{
		printk(KERN_ERR"EMDOOR_JN id=%d\n\n",g_fg_battery_id);
...
#ifdef MTK_MULTI_BAT_PROFILE_SUPPORT
	if (g_fg_battery_id == 1) {
		__batt_meter_parse_node(np, "q_max_pos_byd_50",
			&batt_meter_cust_data.q_max_pos_50);

		__batt_meter_parse_node(np, "q_max_pos_byd_25",
			&batt_meter_cust_data.q_max_pos_25);

		__batt_meter_parse_node(np, "q_max_pos_byd_0",
			&batt_meter_cust_data.q_max_pos_0);

		__batt_meter_parse_node(np, "q_max_neg_byd_10",
			&batt_meter_cust_data.q_max_neg_10);

		__batt_meter_parse_node(np, "q_max_pos_byd_50_h_current",
			&batt_meter_cust_data.q_max_pos_50_h_current);

		__batt_meter_parse_node(np, "q_max_pos_byd_25_byd_h_current",
			&batt_meter_cust_data.q_max_pos_25_h_current);

		__batt_meter_parse_node(np, "q_max_pos_byd_0_h_current",
...
int batt_meter_init_cust_data(void)
{
	...
	init_done = 1;
#ifdef MTK_MULTI_BAT_PROFILE_SUPPORT
	fgauge_get_profile_id();//lsk modify
#endif
...
signed int fgauge_read_capacity_by_v(signed int voltage)
{
    ...
    printk("zheng fgauge_read_capacity_by_v in voltage = %d\n",voltage);
	...
	saddles = fgauge_get_saddles();
    printk("voltage = %d   (profile_p + 0)->voltage = %d   (profile_p + saddles - 1)->voltage = %d \n ",voltage,((profile_p + 0)->voltage),((profile_p + saddles - 1)->voltage));
    ...
    printk("zheng fgauge_read_capacity_by_v return ret_percent = %d\n",ret_percent);
	return ret_percent;
}
...
void dod_init(void)
{
...
#endif				/* #if defined(IS_BATTERY_REMOVE_BY_PMIC) */
	printk("zheng dod_init\n");

	/* use get_hw_ocv----------------------------------------------------------------- */
	ret = battery_meter_ctrl(BATTERY_METER_CMD_GET_HW_OCV, &gFG_voltage);
	gFG_capacity_by_v = fgauge_read_capacity_by_v(gFG_voltage);
	printk("zheng dod_init gFG_voltage = %d  gFG_capacity_by_v = %d\n",gFG_voltage,gFG_capacity_by_v);
	bm_print(BM_LOG_CRTI, " zheng [FGADC] get_hw_ocv=%d, HW_SOC=%d, SW_SOC = %d\n",
	...
	g_rtc_fg_soc = get_rtc_spare_fg_value();
#endif
    printk("zheng dod_init g_rtc_fg_soc = %d  gFG_capacity_by_v = %d\n",g_rtc_fg_soc,gFG_capacity_by_v);
...
if (((g_rtc_fg_soc != 0)
    ...
    printk("dod_init gFG_DOD0=%d gFG_DOD1=%d gFG_capacity = %d  gFG_capacity_by_v = %d gFG_capacity_by_c = %d\n ",gFG_DOD0,gFG_DOD1,gFG_capacity,gFG_capacity_by_v,gFG_capacity_by_c);
...
signed int mtk_imp_tracking(signed int ori_voltage, signed int ori_current, signed int recursion_time)
{
	...
	bm_print(BM_LOG_FULL,
		 "[mtk_imp_tracking] temp_voltage_2=%d,temp_voltage_1=%d,ret_compensate_value=%d,gFG_resistance_bat=%d\n",
		 temp_voltage_2, temp_voltage_1, ret_compensate_value, gFG_resistance_bat);
    printk("[mtk_imp_tracking] temp_voltage_2=%d,temp_voltage_1=%d,ret_compensate_value=%d,gFG_resistance_bat=%d\n",
		 temp_voltage_2, temp_voltage_1, ret_compensate_value, gFG_resistance_bat);
...
void oam_init(void)
{
	...
	printk("zheng BATTERY_METER_CMD_GET_HW_OCV gFG_voltage = %d\n",gFG_voltage);
	gFG_capacity_by_v = fgauge_read_capacity_by_v(gFG_voltage);
	vbat_capacity = fgauge_read_capacity_by_v(g_booting_vbat);
    printk("zheng BATTERY_METER_CMD_GET_HW_OCV gFG_capacity_by_v = %d\n",gFG_capacity_by_v);
//[BUGFIX]-Mod-BEGIN by SCDTABLET.zhengding.chen@tcl.com,2016/11/29,PR3621040
//oam_init hw_ocv/vbat calibration, sync to pixi473g
#if 0
    ...
    if(bat_is_charger_exist() == KAL_TRUE)
    {
        if(g_booting_vbat < 3350)
                ...
                g_booting_vbat = g_booting_vbat + 85;
        ...
        if(gFG_voltage > 3500 && gFG_voltage < 3700)
            gFG_voltage = gFG_voltage - 65;
        else if(gFG_voltage >= 3700 && gFG_voltage < 3900)
            gFG_voltage = gFG_voltage - 70;
        ...
        bm_print(BM_LOG_CRTI, "[oam_init_inf] czd:after calibration :gFG_capacity_by_v=%d,gFG_voltage=%d, vbat_capacity=%d, g_booting_vbat=%d\n",gFG_capacity_by_v, gFG_voltage, vbat_capacity, g_booting_vbat);

        if ((abs(gFG_capacity_by_v - vbat_capacity) > batt_meter_cust_data.cust_poweron_delta_vbat_tolrance) || (gFG_capacity_by_v < vbat_capacity)) {
            gFG_voltage = g_booting_vbat;
            gFG_capacity_by_v = vbat_capacity;
        }
    }
...
#endif
//[BUGFIX]-Mod-END by SCDTABLET.zhengding.chen@tcl.com,2016/11/29,PR3621040
...
#ifdef MTK_MULTI_BAT_PROFILE_SUPPORT
if(g_fg_battery_id == 1){//byd battery 
	if(BMT_status.temperature > 15){
		oam_i_1 = (((oam_v_ocv_1 - vol_bat) * 1000) * 10) / oam_r_1 * 65 / 100;	/* 0.1mA */
		oam_i_2 = (((oam_v_ocv_2 - vol_bat) * 1000) * 10) / oam_r_2 * 65 / 100;	/* 0.1mA */
	}else{
		oam_i_1 = (((oam_v_ocv_1 - vol_bat) * 1000) * 10) / oam_r_1;	/* 0.1mA */
		oam_i_2 = (((oam_v_ocv_2 - vol_bat) * 1000) * 10) / oam_r_2;	/* 0.1mA */
	}

	if((BMT_status.temperature > 15)&&((oam_i_1 < 0)||(oam_i_2 < 0))){
		if(BMT_status.SOC > 29) {
			oam_i_1 = oam_i_1*85/100;
			oam_i_2 = oam_i_2*85/100;
		}
    }
	
    if((BMT_status.temperature <= 10)&&((oam_i_1 > 0)||(oam_i_2 > 0))){
		//if(BMT_status.temperature <= 5){
		//	oam_i_1 = oam_i_1*10/7;
		//	oam_i_2 = oam_i_2*10/7;
		//}else{
			oam_i_1 = oam_i_1*10/9;
			oam_i_2 = oam_i_2*10/9;
		//}		
    }
	
    if((BMT_status.temperature <= 10)&&((oam_i_1 < 0)||(oam_i_2 < 0))&&(BMT_status.SOC >= 43)){
		if(BMT_status.temperature <= 5){
			oam_i_1 = oam_i_1*65/100;
			oam_i_2 = oam_i_2*65/100;
		}else{
			oam_i_1 = oam_i_1*7/10;
			oam_i_2 = oam_i_2*7/10;
		}
    }

	printk("[lsk] oam_run byd g_fg_battery_id = %d\n",g_fg_battery_id);
}else
#endif
{//jn
	oam_i_1 = (((oam_v_ocv_1 - vol_bat) * 1000) * 10) / oam_r_1 * 8 / 10;	/* 0.1mA */
	oam_i_2 = (((oam_v_ocv_2 - vol_bat) * 1000) * 10) / oam_r_2 * 8 / 10;	/* 0.1mA */
	
	/*add by xieheng*/
    if((BMT_status.temperature <= 10)&&((oam_i_1 > 0)||(oam_i_2 > 0))){
		if(BMT_status.temperature <= 5){
			oam_i_1 = oam_i_1*100/55;
			oam_i_2 = oam_i_2*100/55;
		}else{
			oam_i_1 = oam_i_1*10/7;
			oam_i_2 = oam_i_2*10/7;
		}
	...
	#ifdef MTK_MULTI_BAT_PROFILE_SUPPORT
	printk("[lsk] oam_run jn g_fg_battery_id = %d\n",g_fg_battery_id);
	#endif
}
	...
	printk("zheng mtk_imp_tracking oam_v_ocv_1 = %d  oam_d_3 = %d vol_bat = %d oam_i_2 = %d \n",oam_v_ocv_1,oam_d_3,vol_bat,oam_i_2);
	if (oam_d_3 < 0)
		oam_d_3 = 0;
	if (oam_d_3 > 100)
		oam_d_3 = 100;
...
/* 3. Calculate battery capacity by Coulomb Counter */
	gFG_capacity_by_c = fgauge_read_capacity(1);
    printk("zheng fgauge_algo_run_init gFG_capacity_by_c = %d\n",gFG_capacity_by_c);
...
signed int battery_meter_get_charging_current_imm(void)
{
	...
	if(1==ic_vendor){
		//printk("LQ --> %s PSC5415A cust_r_sense=56\n",__func__);
		batt_meter_cust_data.cust_r_sense = 56;
	}	
